...
total_exposure <- example_file %>%
  select(sector, portfolio_sector_share) %>%
  unique()
...
for (result_file_path in result_files_paths) {
  ...
  if (nrow(results) > 0) {
    results <- results %>%
      mutate(
        weight = portfolio_economic_value_share_sector * portfolio_sector_share
      )
    results <- results %>%
      mutate(
        risk_weight = relative_change * weight / total_exposure,
        risk_weight_2 = relative_change * relative_change * weight / total_exposure
      )
    ...
    sector_dist <- results %>%
      group_by(sector, relative_change) %>%
      summarise(
        portfolio_economic_value_share_sector = sum(portfolio_economic_value_share_sector)
      )
    sector_dist_ <- sector_dist %>%
      mutate(
        risk = relative_change * portfolio_economic_value_share_sector,
        risk_2 = relative_change * relative_change * portfolio_economic_value_share_sector
      ) %>%
      group_by(sector) %>%
      summarise(risk = sum(risk), risk2 = sum(risk_2)) %>%
      mutate(std = abs(risk**2 - risk2)**0.5) %>%
      select(-risk2)
    sector_dist_ref <- results %>%
      mutate(
        risk = relative_change / nrow(results),
        risk_2 = risk_level * risk_level / nrow(results)
      ) %>%
      summarise(risk = sum(risk), risk2 = sum(risk_2)) %>%
      mutate(std = abs(risk**2 - risk2)**0.5) %>%
      select(-risk2)
    ...
    value_indicator <- results %>%
      filter(relative_change > value_indicator_ths) %>%
      summarise(value_indicator = sum(weight)) %>%
      mutate(value_indicator = value_indicator / total_exposure)
    ...
    for (sector_ in sectors) {
      mean <- sum(sector_dist_ %>% filter(sector == sector_) %>% select(risk)) %>%
        round(3)
      std <- sum(sector_dist_ %>% filter(sector == sector_) %>% select(std)) %>%
        round(3)
...
for (result_file_path in result_files_paths) {
  ...
  results <- results %>%
    mutate(risk_level = round(risk_level)) %>%
    filter(has_geo_data == TRUE)
  ...
  if (nrow(results) > 0) {
    ...
    results <- results %>%
      mutate(
        risk_weight = risk_level * weight / total_exposure,
        risk_weight_2 = risk_level * risk_level * weight / total_exposure
      )
    ...
    source <- unique(results$provider)
    sector_dist <- results %>%
      group_by(sector, risk_level) %>%
      summarise(
        portfolio_economic_value_share_sector = sum(portfolio_economic_value_share_sector)
      )
    sector_dist_ <- sector_dist %>%
      mutate(
        risk = risk_level * portfolio_economic_value_share_sector,
        risk_2 = risk_level * risk_level * portfolio_economic_value_share_sector
      ) %>%
      group_by(sector) %>%
      summarise(risk = sum(risk), risk2 = sum(risk_2)) %>%
      mutate(std = abs(risk**2 - risk2)**0.5) %>%
    ...
    value_indicator <- results %>%
      filter(risk_level > value_indicator_ths) %>%
      summarise(value_indicator = sum(weight)) %>%
      mutate(value_indicator = value_indicator / total_exposure)
    value_indicator <- sum(value_indicator$value_indicator) %>% round(3)
